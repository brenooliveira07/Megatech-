...
CREATE POLICY "si_select_own" ON public.sale_items FOR SELECT TO authenticated
USING (
  EXISTS (
...
CREATE POLICY "sm_select_admin" ON public.stock_movements FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "sm_insert" ON public.stock_movements;
CREATE POLICY "sm_insert_admin" ON public.stock_movements FOR INSERT TO authenticated
...
DROP TRIGGER IF EXISTS sale_item_enforce_price ON public.sale_items;
CREATE TRIGGER sale_item_enforce_price
BEFORE INSERT ON public.sale_items
FOR EACH ROW EXECUTE FUNCTION public.enforce_sale_item_price();
...
BEGIN
  ...
  UPDATE public.sales
  SET total = COALESCE((SELECT SUM(subtotal) FROM public.sale_items WHERE sale_id = target_sale), 0)
...
DROP TRIGGER IF EXISTS sale_item_recompute_total ON public.sale_items;
