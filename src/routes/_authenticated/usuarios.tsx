...
function Usuarios() {
  ...
  async function saveRole() {
    ...
    if (roleRow.id === auth.user?.id) {
      window.dispatchEvent(new Event("megatech:user-data-changed"));
      if (newRole !== "admin" && newRole !== "gerente") {
                      ...
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button size="icon" variant="ghost" aria-label={`Ações de ${u.full_name ?? u.email ?? "usuário"}`}>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            ...
                            <DropdownMenuItem
                              disabled={(!canManageThisUser && !isSelf) || (isSelf && !auth.isAdmin)}
      ...
      <Dialog open={!!roleRow} onOpenChange={(o) => !o && setRoleRow(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveRole} disabled={savingRole || newRole === roleRow?.roles[0]}>
